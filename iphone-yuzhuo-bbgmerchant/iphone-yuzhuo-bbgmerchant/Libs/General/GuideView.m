...
{
    if (i == -1) {
        showRect = CGRectMake(self.bounds.size.width/2- 100, self.bounds.size.height/2, 1, 1);
    }
    _showRect = showRect;
...
- (void)setMessageArray:(NSArray *)messageArray rectArray:(NSArray *)rectArray {
    i++;
    NSLog(@"%d",i);
    if (i < rectArray.count && i < messageArray.count) {
        self.showRect = [rectArray[i] CGRectValue];
        self.markText = messageArray[i];
    }else{
